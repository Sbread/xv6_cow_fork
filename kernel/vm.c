...
int uvmcopy_cow(pagetable_t old, uint64 addr, int flag) {
  pte_t *pte;
  uint64 pa;
  uint flags;
  char *mem;
  ...
  if (pte == 0 || (*pte & PTE_V) == 0) {
    ...
    memmove(mem, (char *)pa, PGSIZE);
    uvmunmap(old, PGROUNDDOWN(addr), 1, 1);
  }
  if (mappages(old, PGROUNDDOWN(addr), PGSIZE, (uint64)mem, flags) != 0) {
    kfree(mem);
    goto err;
  }
  return 0;
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    if (uvmcopy_cow(pagetable, dstva, 0) == -1) return -1;
...
int copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len) {
  ...
  while (len > 0) {
    ...
    if (va0 >= MAXVA) return -1;
    if (uvmcopy_cow(pagetable, va0, 0) == -1) return -1;
    pa0 = walkaddr(pagetable, va0);
    if (pa0 == 0) return -1;
    n = PGSIZE - (srcva - va0);
    if (n > len) n = len;
